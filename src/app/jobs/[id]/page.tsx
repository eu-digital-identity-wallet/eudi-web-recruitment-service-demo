      ...
      >
        ...
        <CardContent sx={{ pt: 0, '&:last-child': { pb: 2 } }}>
          <Box sx={{ display: 'flex', flexDirection: { xs: 'column', lg: 'row' }, gap: 3, alignItems: 'stretch' }}>
            ...
            {/* Apply Section */}
            <Box sx={{ flex: 1, display: 'flex' }}>
              <Box sx={{ p: 2, border: 1, borderColor: 'divider', borderRadius: 0, display: 'flex', flexDirection: 'column', gap: 2, width: '100%' }}>
                  ...
                  {(job.getRequiredCredentials() !== 'NONE' && job.getRequiredCredentials() !== 'PID') && (
                    <Box sx={{ mb: 2 }}>
                      <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
                        Optional Credentials:
                      </Typography>
                      <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                        {(job.getRequiredCredentials() === 'DIPLOMA' || job.getRequiredCredentials() === 'BOTH') && (
                          <Chip color="primary" variant="outlined" label="Diploma (optional)" />
                        )}
                        {(job.getRequiredCredentials() === 'SEAFARER' || job.getRequiredCredentials() === 'BOTH') && (
