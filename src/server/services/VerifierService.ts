...
console.log("Init verification called with:", { applicationId, sameDeviceFlow, credentialType });
    ...
    const payload: VpTokenRequest = buildVpTokenRequest({
      applicationId,
...
console.log("Verifier response status:", response.status);
  ...
  private extractClaimsFromNamespace(ns: unknown): Record<string, unknown> {
    ...
    for (const entry of ns) {
      ...
      if (!de || typeof de !== 'object' ||
          !(de as { elementIdentifier?: string }).elementIdentifier) continue;
  ...
  private processPidClaims(claims: Record<string, unknown>) {
    ...
    } else if (typeof birthDateClaim === "object" && birthDateClaim !== null && "value" in birthDateClaim) {
  ...
  public async checkVerification(
    ...
    try {
      ...
      if (!decoded || typeof decoded !== 'object' ||
          ...
          !(decoded as { documents: { issuerSigned?: { nameSpaces?: unknown } }[] }).documents[0]?.issuerSigned?.nameSpaces) {
        return { status: false };
      }
      console.log("VP token decoded");
      //Extract all namespaces
      const decodedData = decoded as { documents: { issuerSigned: { nameSpaces: Record<string, unknown> } }[] };
