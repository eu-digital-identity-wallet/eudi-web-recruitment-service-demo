...
export class ContractPdfGeneratorService {
	...
	async generateEmploymentContract(contractData: {
		...
		// Employee signature
		page.drawLine({
			start: { x: this.MARGIN_LEFT, y: signatureY },
			end: { x: this.MARGIN_LEFT + 200, y: signatureY },
			thickness: 1,
			color: this.COLOR_TEXT,
		});

		drawText('Employee Signature (Digital)', {
			size: 9,
			color: this.COLOR_SECONDARY,
			lineHeight: 12,
		});

		drawText(candidateName, {
			size: 10,
			lineHeight: 14,
		});
		...
		page.drawLine({
			start: { x: this.PAGE_WIDTH - this.MARGIN_RIGHT - 200, y: signatureY },
			end: { x: this.PAGE_WIDTH - this.MARGIN_RIGHT, y: signatureY },
			thickness: 1,
			color: this.COLOR_TEXT,
		});
		...
		// Footer
		const footerY = this.MARGIN_BOTTOM + 20;
		page.drawText(`Generated on ${contractDate} | Reference: ${contractData.applicationId}`, {
			x: this.MARGIN_LEFT,
			y: footerY,
			size: 8,
			font: regularFont,
