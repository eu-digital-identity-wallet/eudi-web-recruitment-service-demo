...
export class ApplicationService {
	...
	public async create(newRequesParamsFor: {
		jobId: string;
	...
	public async issueReceipt(
		applicationId: string,
	): Promise<{ offerUrl: string; otp?: string } | null> {
		...
		const offerResponse = await this.issuer.issueApplicationReceipt(
			prismaApp as typeof prismaApp & { job: NonNullable<typeof prismaApp.job> },
		);
		...
		// Persist domain changes
		await this.applicationRepo.update(applicationId, ApplicationMapper.toPersistence(application));
